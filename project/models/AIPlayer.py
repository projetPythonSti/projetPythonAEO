...
class AIPlayer:
    ...
    def playTurn(self):
        ...
        if workingPpl < self.playStyle.minWorkers:
            ...
            numberOfActions = 0
            # Fixed loop to iterate over a copy of eventQueue
            for action in self.eventQueue[:]:
                numberOfActions += 1
    ...
    def launchAction(self, actionDict):
        # Added type check to handle non-dict events
        if not isinstance(actionDict, dict):
            self.logger("Invalid actionDict encountered, skipping.")
            return
