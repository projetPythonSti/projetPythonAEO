...
class World_GUI:
    def __init__(self, entities, grid_width, grid_height, width, height, world: World):
        ...
        self.grass_tiles = pg.Surface(
            (self.grid_width * TILE_SIZE * 2, self.grid_height * TILE_SIZE + 2 * TILE_SIZE)).convert_alpha()
        self.tile_images = defaultdict(None)
        self.all_withouth_ressources = {**self.world_model.villages[0].population(), **self.world_model.villages[
            1].population()}  # Removed self.world_model.ressources
        self.all = {**self.world_model.villages[0].population(), **self.world_model.villages[1].population(),
                    **self.world_model.ressources}
        # logger.debug(f"World_GUI: all entities: {self.all}")
        self.load_images()
        self.create_world()
        self.world_grid = self.world_model.filled_tiles
        self.i = 0
        self.keeps = [kv for k, v in self.all.items() for kv in v.values() if
                      isinstance(kv, Building) and kv.name == "K"]
        ...
        for (k, ressources) in self.world_model.get_ressources().items():
            for (_, ressource) in ressources.items():
                if ressource:
                    ...
                    #print(
                    #    f"Drawing ressource {image_key} at position: {ressource.getTPosition()}")  # Added debug statement
    ...
    def mouse_to_grid(self, x, y, camera):
        ...
        adjusted_x = (x - camera.scroll.x * camera.zoom - (
                    self.grass_tiles.get_width() / 2) * camera.zoom) / camera.zoom
    ...
    def place_building(self, grid_pos):
        ...
        if self.hud and self.hud.selected_tile:
            ...
            if selected_building_name in self.hud.building_classes:
                ...
                print(
                    f"Attempting to place {building.name} at ({grid_pos[0]}, {grid_pos[1]}) with size ({size_x}, {size_y})")  # Debug statement
                ...
                if self.can_place_building(grid_pos[0], grid_pos[1], size=building.size):
                    ...
                    print(
                        f"Cannot place {building.name} at ({grid_pos[0]}, {grid_pos[1]}): Collision or out of bounds.")  # Debug statement
        else:
