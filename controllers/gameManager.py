...
class GameManager:
    ...
    def checkModifications(self):
        ...
        self.logger("--------- End Of CUTA ---------"),
        self.checkUnitToSpawn()
        self.logger("--------- End Of CUTS ---------"),
    ...
    def moveUnit(self, uid):
        ...
        if unit["timeElapsed"] >= (unit["timeToTile"]):
            ...
            self.world.villages[(unit["team"] - 1)].community[(unit["type"].lower())][uid].position = Position(
                unit["moveQueue"][0][0], unit["moveQueue"][0][1])
            self.logger("GameManager | moveUnit----- My position is ,",
                        self.world.villages[(unit["team"] - 1)].community[(unit["type"].lower())][uid].position)
    ...
    def buildBuilding(self, uid):
        ...
        for unit in building["units"]:
            readyToBuild = self.unitNear(
                self.world.villages[building["team"] - 1].community["v"][unit].position.toTuple(), building["nearTile"],
                building["type"])
            if readyToBuild:
                self.world.villages[building["team"] - 1].community[building["type"]][
                    uid].builders += 1 if buildingInstance.builders <= len(building["units"]) else 0
            else:
                self.world.villages[building["team"] - 1].community[building["type"]][
                    uid].builders -= 1 if buildingInstance.builders > 0 else buildingInstance.builders
        timeToBuild = 3 * building["nominalBuildTime"] / (
                    buildingInstance.builders + 2) if buildingInstance.builders > 0 else -1
        ...
        if timeToBuild > 0:
            if building["timeElapsed"] > timeToBuild:
                ...
                buildingInstance.health += deltaTime * self.gameSpeed * buildingHealthENUM[
                    building["type"]].value / timeToBuild
    ...
    def dumbCollectResources(self, uid):
        ...
        realResQuantity = resToCollect["resourceQuantity"] if resToCollect["resourceQuantity"] <= \
                                                              self.world.ressources[resToCollect["resourceType"]][
                                                                  resToCollect["resourceID"]].quantity else \
        self.world.ressources[resToCollect["resourceType"]][resToCollect["resourceID"]].quantity
        ...
        if resDistance <= (1, 1) and not resToCollect["full"]:
            ...
            if resToCollect["timeElapsed"] < timeToFillPouch and not resToCollect["full"]:
                ...
                quantityToCollect = unitSpaceLeft if unitSpaceLeft <= resToCollect["resourceQuantity"] else \
                resToCollect["resourceQuantity"]
            ...
            if resToCollect["full"]:
                ...
                self.dumbAddUnitToMoveDict(unitInstance,
                                           Position(resToCollect["nearDPPos"][0], resToCollect["nearDPPos"][1]))
            ...
            if resToCollect["full"]:
                ...
                if dpDistance <= (1, 1):
                    ...
                    if resToCollect["finished"]:
                        ...
                        self.dumbAddUnitToMoveDict(unitInstance, Position(resToCollect["resourceTarget"][0],
                                                                          resToCollect["resourceTarget"][1]))
                else:
    ...
    def attackUnit(self, uid):
        ...
        self.logger("TEST ATTACKING", attackingUnit)
        ...
        if attackingUnit["targetID"] in self.world.villages[attackingUnit["targetTeam"] - 1].deads:
            ...
            return -1
        targetInstance = self.world.villages[attackingUnit["targetTeam"] - 1].community[attackingUnit["targetType"]][
            attackingUnit["targetID"]]
        self.logger("GameManager | attackUnit--- Quel type ?", targetInstance.name)
        self.logger("Game manager attackUnit--- Position unité,: ", attackingUnitInstance.position,
                    "Position à atteindre :", targetInstance.position)
        if attackingUnit["movingTarget"]:
            self.logger("Game manager attackUnit--- Element qui bouge")
            if targetPosition != targetInstance.position.toTuple() and attackingUnitInstance.estimateDistance(
                    targetPosition, targetInstance.position.toTuple()) < (10, 10):
                pass
                """
        ...
        if attackingUnitInstance.isInRange(targetPosition):
            ...
            if not (attackingUnit["targetInRange"]):
                ...
                self.attackDict[attackingUnit["targetTeam"]] += [
                    (attackingUnit["targetType"], attackingUnitInstance, attackingUnitInstance.team.name)]
            ...
            if targetInstance.health < 0:
                    ...
                    for a in self.buildingsToBuild.keys():
                        ...
                        if self.getTeamNumber(a) == attackingUnit["targetTeam"]:
                            ...
                            for unit in unitToRemove:
                                self.logger("GameManager | attackUnit--- should pop targetInstance in buildingsToBuild",
                                            unit)
                ...
                self.world.villages[attackingUnit["targetTeam"] - 1].community[attackingUnit["targetType"]][
                    attackingUnit["targetID"]].health -= attackingUnitInstance.damage * gameDeltaTime
        else:
    ...
    def dumbAttackUnit(self, uid):
        ...
        targetUnitInstance = \
        self.world.villages[attackingUnitDict["targetTeam"] - 1].community[attackingUnitDict["targetType"]][
            attackingUnitDict["targetID"]]
        if attackingUnitInstance.isInRange(targetUnitInstance.position.toTuple()):
            if not (attackingUnitDict["targetInRange"]):
                self.attackDict[int(attackingUnitDict["targetTeam"])] += [
                    (attackingUnitDict["targetID"], attackingUnitInstance)]
                self.logger("GameManager | collectResources--- Waiting to arrive to Resource")
            ...
            if targetUnitInstance.health <= 0:
                ...
                self.world.villages[attackingUnitDict["targetTeam"] - 1].community[attackingUnitDict["targetType"]][
                    attackingUnitDict["targetID"]].health -= attackingUnitInstance.damage * gameDeltaTime

        else:
            self.addUnitToAttackDict(attackingUnitInstance, targetUnitInstance.position)

    def spawnUnit(self, uid):
        deltaTime = (timeit.default_timer() - self.tick)
        gameDeltaTime = deltaTime * self.gameSpeed
        unitToSpawn = self.unitToSpawn[uid]
        if unitToSpawn["timeElapsed"] >= unitToSpawn["timeToTrain"]:
            self.logger("GM | AAAAAspawnUnit--- unit is ready !",)
            teamNumber = int(unitToSpawn["unitTeam"])
            self.logger("GM | spawnUnit---- teamnubmer",teamNumber)

            self.world.villages[teamNumber-1].add_unit(unitToSpawn["unitType"],unitToSpawn["unitPos"], futureID=unitToSpawn["unit"])
            unitToSpawn["finished"] = True
        else:
            self.logger("GM | spawnUnit--- waiting for unit to spawn !, time :",unitToSpawn["timeElapsed"])
            unitToSpawn["timeElapsed"] += gameDeltaTime
        ...
        for k in self.ressourceToCollect:
            if self.ressourceToCollect[k]["finished"]:
                ...
                try:
                    ...
                    self.logger("ERREURAU NIVEAU CRTC")
        if resToDelete != "":
    ...
    def checkUnitToSpawn(self):
        unitToDelete = []
        for k in self.unitToSpawn:
            self.logger("GM | checkUnitToSpawn checking unit ", k)
            if self.unitToSpawn[k]["finished"]:
                unitToDelete.append(k)
            else:
                pass
                self.spawnUnit(k)
        for k in unitToDelete:
            self.unitToSpawn.pop(k)
    ...
    def getNearTiles(self, size, position):
        ...
        nearTiles = {tile for tile in nearTiles if
                     (0 < tile[0] < self.world.width) and (0 < tile[1] < self.world.height) and (
                                 self.world.tiles_dico[tile].contains is None)}
    ...
    def addRessourceToCollectDict(self, unit, resource: Ressource, quantity, nearDP):
        ...
        grid = self.world.convertMapToGrid()
        pathFinding = Pathfinding(mapGrid=grid, statingPoint=nearDP.position.toTuple(),
                                  goal=resource.position.toTuple(), debug=False)
        path = pathFinding.astar()
        if path.__class__ == bool:
    ...
    def dumbAddUnitToSpawnDict(self,unitType,position,team, futureID):
        self.logger("DAUTSD -----VOICI LA data", )
        timeToBuild = 0
        self.unitToSpawn[futureID] = {
            "unit": futureID,
            "unitTeam": team,
            "unitType" : unitType,
            "unitPos" : position,
            "timeElapsed": 0,
            "timeToTrain": unitTimeENUM[unitType].value,
            "finished": False,
            "error": False
        }
